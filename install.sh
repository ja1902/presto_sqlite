...
if [ "$presto_choice" = "1" ]; then
    ...
    if [ -d "$PRESTO_HOME" ]; then
        ...
        # Check available disk space (need ~1.5 GB for download + extraction)
        if command -v df &>/dev/null; then
            avail_kb=$(df --output=avail "$install_dir" 2>/dev/null | tail -1 || df -k "$install_dir" 2>/dev/null | tail -1 | awk '{print $4}')
            avail_kb=$(echo "$avail_kb" | tr -d ' ')
            if [ -n "$avail_kb" ] && [ "$avail_kb" -lt 1500000 ] 2>/dev/null; then
                echo "Error: Not enough disk space. Need ~1.5 GB, have ~$((avail_kb / 1024)) MB."
                exit 1
            fi
        fi
...
# Step 6: Start PostgreSQL via Docker (if Docker is available)
...
if command -v docker &>/dev/null; then
    ...
    if [ "$docker_running" = true ]; then
        # Create shared network (ignore if exists)
        docker network create presto-net 2>/dev/null || true

        # Remove existing container for a clean start
        ...
        if [ "$pg_ready" = true ]; then
            ...
            echo "Seeding PostgreSQL demo database..."
            "$VENV_DIR/bin/python" "$SCRIPT_DIR/demo/create_postgres_db.py"
        else
            echo "WARNING: PostgreSQL did not become ready in time."
            echo "Check logs with: docker logs presto-postgres"
        fi

        # Write postgres catalog config for Presto
        cat > "$CATALOG_DIR/postgres.properties" <<'EOF'
connector.name=postgresql
connection-url=jdbc:postgresql://localhost:5432/demo
connection-user=presto
connection-password=presto
EOF
        echo "Catalog config written to $CATALOG_DIR/postgres.properties"
    else
        echo ""
        echo "Docker is installed but not running. Skipping PostgreSQL setup."
        echo "To set up PostgreSQL later, start Docker and re-run this script."
    fi
else
    echo ""
    echo "Docker not found. Skipping PostgreSQL setup."
    echo "Install Docker and re-run this script for cross-catalog query support."
fi
...
if command -v ss &>/dev/null; then
    if ss -tlnp 2>/dev/null | grep -q ':8080 '; then
        ...
        echo "Presto will not be started. Stop whatever is using port 8080 and try:"
        echo "  $PRESTO_HOME/bin/launcher start"
        echo ""
        echo "Done (Presto not started due to port conflict)."
        exit 0
    fi
