...
def main():
    ...
    # PostgreSQL may not be set up (requires Docker). Try it and skip gracefully.
    has_postgres = False
    try:
        run_query(cur, 'SHOW TABLES FROM postgres.public', "Tables in postgresql")
        has_postgres = True
    except PrestoUserError as e:
        if 'postgres' in str(e).lower():
            print("\nPostgreSQL catalog not available -- skipping PostgreSQL and cross-catalog queries.")
            print("To enable: install Docker and re-run the install script.")
        else:
            raise
